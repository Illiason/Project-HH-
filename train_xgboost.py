# Train XGBoost model to predict heart disease
...
# XGBClassifier = the XGBoost machine learning model for classification (yes/no predictions)
from xgboost import XGBClassifier

print("=" * 70)
print("TRAINING XGBoost MODEL - Heart Disease Prediction")
...
model = XGBClassifier(
    ...
    max_depth=5,            # each tree can be 5 levels deep (not too complex)
    learning_rate=0.1,      # each tree makes small corrections (0.1 = 10% contribution)
    random_state=123,        # same results every time we run
    eval_metric='logloss'   # measure errors using log loss (good for yes/no predictions)
)

print("\nTraining XGBoost model...")
# fit() = "learn from this data" - model studies 734 patients and finds patterns
model.fit(X_train, y_train)
print("Training complete!")

# 5. Make predictions
# predict() = "use what you learned to predict these 184 new patients"
# y_pred = list of predictions (0 or 1) for each test patient
y_pred = model.predict(X_test)

# 6. Evaluate model
# Compare predictions (y_pred) with actual answers (y_test)
# accuracy = percentage of correct predictions (e.g., 0.875 = 87.5%)
